...
from als_deprecated.socketio_config import socketio_serializer_mode
...
def frontend_static_root(package_root: Path) -> Path:
    candidates = [
        package_root / "static",
        package_root.parent / "rust" / "crates" / "als-server" / "src" / "static",
        package_root.parent
        / "agent_log_server_rs"
        / "rust"
        / "crates"
...
def _asset(package_root: Path, url: str) -> str:
    ...
    path = frontend_static_root(package_root) / rel
    try:
...
def _codex_agent_version(package_root: Path) -> str:
    static_root = frontend_static_root(package_root)
    paths = [
        Path(__file__),
        static_root / "codex_agent.css",
        static_root / "dist" / "codex_agent.js",
        static_root / CODEX_AGENT_ICON_PATH.removeprefix("/static/"),
    ]
    parts: List[str] = []
    for path in paths:
        try:
