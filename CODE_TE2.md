...
- **TextMate grammar pipeline** (implemented): The workbench adapter now detects, parses, and serves TextMate grammars (`.tmLanguage.json`) over the Node workbench adapter's stdio JSON-RPC transport. Grammar queries (`vscode.grammars.list`, `vscode.grammars.load`) flow through the same pipe as language intelligence — no HTTP, no separate process.
- **vscode_api WebSocket harness fully deprecated**: The standalone `vscode_api` WS JSON-RPC bridge and its associated WebSocket transport have been fully deprecated and removed. All functionality previously served by the WebSocket harness (grammar loading, theme queries, bootstrap snapshots) now routes through the workbench adapter's stdio pipe.
